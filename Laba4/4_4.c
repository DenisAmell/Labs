...
int read_string(FILE *file, char **result, const char *separators, char *metSeparator)
{
    int isEOF = 0;
    int resCapacity = 1;
...
int funcLoad(List ***lists, int *len, char *string)
{ // string: "A, in.txt"
    if (strlen(string) > 50)
    ...
    while (statusCode != -1)
    {
        statusCode = read_string(data, &num, " \n\t", NULL);
        if (statusCode == 2)
        {
            continue;
        }
        else if (statusCode == NOT_MEMORY)
        {
            fclose(data);
            listDestroy(list);
            return statusCode;
...
int funcSave(List **lists, int len, char *string)
{ // string: "A, out.txt"
    if (strlen(string) > 50)
        return INPUT_ERROR;
    int statusCode = 0;
...
int funcRand(List ***lists, int *len, char *string)
{ // string: "A, 10, 2, 6"
    int count, a, b;
    int i[3];
...
int funcConcat(List **lists, int len, char *string)
{ // string: "A, B"
    if (strlen(string) != 4)
        return INPUT_ERROR;
...
int funcRemove(List **lists, int len, char *string)
{ // string: "A, 2, 7"
    int index, count;
    int i[2];
...
int funcCopy(List ***lists, int *len, char *string)
{ // string: "A, 4, 10, B"
    int a, b;
    int i[2];
    int statusCode = parseInts(i, 2, string + 3);
...
int funcSort(List **lists, int len, char *string)
{ // string: "A+"
    List *list = listsSearch(lists, len, string[0]);
...
int funcShuffle(List **lists, int len, char *string)
{ // string: "A"
    List *list = listsSearch(lists, len, string[0]);
    if (list == NULL)
        return NO_SEARCH;
...
int funcStats(List **lists, int len, char *string)
{ // string: "A"
    List *list = listsSearch(lists, len, string[0]);
    if (list == NULL || list->len == 0)
        return NO_SEARCH;
...
int funcPrint(List **lists, int len, char *string)
{ // string: "A, 4, 16" or "A, all"
    List *list = listsSearch(lists, len, string[0]);
    if (list == NULL)
        return NO_SEARCH;
    if (string[3] == 'a')
...
int main(int argc, char *argv[])
{
    ...
    {
        char metSep;
        statusCode = read_string(data, &command, " ;", &metSep);
        if (statusCode == NOT_MEMORY)
        {
        ...
        else if (statusCode == ERROR_OPENING_FILE)
        {
            printf("Error opening file");
            listsDestroy(lists, len);
            fclose(data);
        ...
        {
            statusCode = read_string(data, &args, ";", NULL);
            if (statusCode != 0)
            {
                free(command);
                listsDestroy(lists, len);
