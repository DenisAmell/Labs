...
int main()
{
    ...
    memory *allocator = new border_descriptors_memory(log, nullptr, 10000, memory::allocate_mode::best_fit);

    associative_container<int, int> *bst = new binary_search_tree<int, int, key_comparer>(allocator, log);
    ...
    auto tree = *reinterpret_cast<binary_search_tree<int, int, key_comparer> *>(bst);
