    ...
    while (curr_block != nullptr)
    {
        ...
        if (curr_block_size >= requested_block_size + occup_block_serv_size)
        {
            if (alloc_method == memory::allocate_mode::first_fit ||
                alloc_method == memory::allocate_mode::best_fit &&
                    (target_block == nullptr || get_block_avail_size(target_block) > curr_block_size) ||
                alloc_method == memory::allocate_mode::worst_fit &&
                    (target_block == nullptr || get_block_avail_size(target_block) < curr_block_size))
            {
...
void memory_buddy_system::deallocate(void *block_to_deallocate_address) const
{
    ...
    //     curr_block = get_pointer_avail_next_block(curr_block);
    // }
    ...
    if (buddies != nullptr && (get_block_avail_size(curr_block) == get_block_avail_size(buddies)) && (is_free_block(buddies) == false))
    {
        while (buddies != nullptr && (get_block_avail_size(buddies) == get_block_avail_size(curr_block)) && (is_free_block(buddies) == false))
        {
            if (buddies < curr_block)
            {
                ...
                //*reinterpret_cast<unsigned char *>(buddies) = compact_block_size_and_block_occup(get_block_avail_size(buddies), false);
                // std::cout << "Before" << get_block_avail_size(buddies) << std::endl;
                *reinterpret_cast<unsigned char *>(buddies) = ((*reinterpret_cast<unsigned char *>(buddies) >> 1) + 1) << 1;
                // std::cout << "After" << get_block_avail_size(buddies) << std::endl;
                curr_block = buddies;
            }
            else
            {
                ...
                // *reinterpret_cast<unsigned char *>(curr_block) = compact_block_size_and_block_occup(static_cast<size_t>(std::log2(get_block_avail_size(curr_block))), false);
                ...
                *reinterpret_cast<void **>(reinterpret_cast<unsigned char *>(curr_block) + 1 + sizeof(void *)) = next_block;
                ...
                // std::cout << is_free_block(curr_block) << std::endl;
                if (next_block != nullptr)
                {
...
memory_buddy_system::memory_buddy_system(logger *log, memory *allocator, size_t size_as_power_of_2, allocate_mode method)
{
    // int16_t temp = 127 << 1;
    // char value = temp;
    // std::cout << "Битовое представление числа "
    //           << value
    //           << " = " << std::bitset<sizeof(value) * CHAR_BIT>(value) << std::endl;
