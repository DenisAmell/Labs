...
public:
    bool can_execute(std::string const &request) noexcept final
    {
        ...
        if (!prefix_validation("GET_DATA_BETWEEN", request))
            return false;
    ...
    {
        database_singleton::get_instance()->get_data_between_keys(_pool_name, _scheme_name, _collection_name, _min_key, _max_key);
